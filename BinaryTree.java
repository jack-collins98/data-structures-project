...
public class BinaryTree<T> {
	...
	*/
	...
	class BinaryNode<T>{
		...
		public String breadthFirstTraverse(){
			q.add(root); //add a node to the queue
		...
		BinaryNode x;
			...
			if(x.leftNode != null) { //if the left node is not empty add node to the linked list
				q.add(x.leftNode);
			}
			if(x.rightNode != null) { //if the right node is not empty add node to the linked list
				q.add(x.rightNode);
				} 
			} // end while loop
			return str;
